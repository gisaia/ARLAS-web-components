...
import { CirclePaintProps, FilterSpecification, LayerSpecification } from 'maplibre-gl';
...
@Component({
  selector: 'arlas-maplibregl-legend',
  templateUrl: './mapgl-legend.component.html',
  styleUrls: ['./mapgl-legend.component.scss']
})
export class MaplibreglLegendComponent implements OnInit, AfterViewInit, OnChanges {
  /**
   * @Input : Angular
   * @description Layer object
   */
  @Input() public layer: LayerSpecification;
  /**
   * @Input : Angular
    ...
    this.visibilityUpdater
      ...
      .subscribe(visibilityUpdater => {
        ...
        if (this.layer && !this.layer.id.startsWith(ARLAS_ID) &&
          ...
          if (!!this.layer.metadata && (<any>this.layer.metadata).showLegend === false) {
            this.visibleMode = false;
          }
        }
        if (!!this.layer) {
  ...
  private drawLegends(visibileMode: boolean): void {
    ...
    switch (type) {
      case 'circle': {
        const p: any = (paint as maplibregl.CirclePaintProps);
        const colors = MaplibreglLegendComponent.buildColorLegend(p['circle-color'], visibileMode,
          this.legendData, this.layer.filter as LegacyFilterSpecification, this.translate);
        const strokeColors = MaplibreglLegendComponent.buildColorLegend(p['circle-stroke-color'], visibileMode, this.legendData,
          this.layer.filter as LegacyFilterSpecification, this.translate);
      ...
      case 'line': {
        const p: any = (paint as maplibregl.LinePaintProps);
        const colors = MaplibreglLegendComponent.buildColorLegend(p['line-color'], visibileMode,
          this.legendData, this.layer.filter as LegacyFilterSpecification, this.translate);
        this.buildWidthLegend(p['line-width']);
         // TODO: linedash if type cross faded props but the classe is not exported check how it behave
      ...
      case 'fill': {
        const p: any = (paint as maplibregl.FillPaintProps);
        const colors = MaplibreglLegendComponent.buildColorLegend(p['fill-color'], visibileMode, this.legendData,
          this.layer.filter as LegacyFilterSpecification, this.translate);
        this.colorLegend = colors[0];
        this.colorPalette = colors[1];
        if (!!metadata && !!(<any>metadata).stroke) {
          const strokeColors = MaplibreglLegendComponent.buildColorLegend((<any>metadata).stroke.color, visibileMode, this.legendData,
            this.layer.filter as LegacyFilterSpecification, this.translate);
          this.strokeColorLegend = strokeColors[0];
          this.strokeColorPalette = strokeColors[1];
        }
      ...
      case 'heatmap': {
        const p: any = (paint as maplibregl.HeatmapPaintProps);
        const colors = MaplibreglLegendComponent.buildColorLegend(
          p['heatmap-color'], visibileMode, this.legendData, this.layer.filter as LegacyFilterSpecification, this.translate
        );
      ...
      case 'symbol': {
        ...
        const colors = MaplibreglLegendComponent.buildColorLegend(p['text-color'], visibileMode,
          this.legendData, this.layer.filter as LegacyFilterSpecification, this.translate);
        this.colorLegend = colors[0];
        this.colorPalette = colors[1];
        const l: any = (paint as maplibregl.SymbolLayoutProps);
  ...
  public static filterLegend(colorLegendValues: Map<string, string | number>, filter: LegacyFilterSpecification, field: string) {
  ...
  public static buildColorLegend(colorExpression: string | maplibregl.StylePropertyFunction<any> | maplibregl.Expression
                                   | any, visibleMode: boolean, legendData: Map<string, LegendData>,
                                 filter?: LegacyFilterSpecification, translate?: TranslateService): [Legend, string] {
    ...
    if (typeof colorExpression === 'string') {
      ...
      if (colorExpression.length === 2) {
        ...
        if (!Array.isArray(field)) {
          ...
          if (!!filter) {
            MaplibreglLegendComponent.filterLegend(colorLegend.manualValues, filter,
        ...
        if (colorExpression[0] === MATCH) {
          ...
          if (!!filter) {
            MaplibreglLegendComponent.filterLegend(colorLegend.manualValues, filter, field);
          }
  ...
  private buildWidthLegend(lineWidth: number | maplibregl.StylePropertyFunction<any> | maplibregl.Expression): void {
  ...
  private buildCircleRadiusLegend(circleRadius: number | maplibregl.StylePropertyFunction<any> | maplibregl.Expression): void {
